...
try {
    ...
    ResultSet rs = st.executeQuery("select * from userdata where email= '"+email+"' and pass='"+pass+"'");
    if(rs.next()) {
    String user = rs.getString(2);
  	session.setAttribute("name",email);
    response.sendRedirect("stu_home.jsp?n1=success");
	}
    else {
    response.sendRedirect("stu_login.jsp?n2=LoginFail");
    }
}
catch(Exception e) {
